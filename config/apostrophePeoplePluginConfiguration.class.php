...
class apostrophePeoplePluginConfiguration extends sfPluginConfiguration
{
  ...
  public function migrate($event)
  {
    ...
    {
      $migrate->sql(array(
        'CREATE TABLE a_person (id BIGINT AUTO_INCREMENT, first_name VARCHAR(255) NOT NULL, middle_name VARCHAR(255), last_name VARCHAR(255) NOT NULL, salutation VARCHAR(255), suffix VARCHAR(255), email VARCHAR(255) NOT NULL, address_1 VARCHAR(255), address_2 VARCHAR(255), city VARCHAR(255), state VARCHAR(255), postal_code VARCHAR(255), work_phone VARCHAR(255), work_fax VARCHAR(255), body TEXT, link VARCHAR(255), sex VARCHAR(1) DEFAULT NULL, ethnicity VARCHAR(255), headshot_id BIGINT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, slug VARCHAR(255), UNIQUE INDEX a_person_sluggable_idx (slug), INDEX headshot_id_idx (headshot_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ENGINE = INNODB;',
